...
void calc_window(unordered_set<int> &sum_in_arr, int *arr, int n) {
    int prev[n];
    ...
    for (int len = 2; len < n+1; len++) {
        for (int i = 0; i < n-(len-1); i++) {
            int wsum = prev[i] + arr[i+len-1];
            if (arrset.find(wsum) != arrset.end()) {
                sum_in_arr.ins(wsum);
            }
            prev[i] = wsum;
        }
...
inline void _solve_() {
    ...
    calc_window(sum_in_arr, arr, n);

    int tot = 0;
    FORI(n) {
        if (sum_in_arr.find(arr[i]) != sum_in_arr.end()) {
            tot++;
        }
    }
